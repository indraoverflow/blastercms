...
foreach ($r->result() as $d){
    $data[$i] = array(
                    '<input type="checkbox" class="cekbox" name="cekbox[]" value="'.$d->WidgetID.'" />',
                    $d->WidgetID,
                    anchor('widget/edit/'.$d->WidgetID, $d->WidgetName)
                );
    $i++;
}
...
<p>
    <?=anchor('widget/delete','<b class="glyphicon glyphicon-trash"></b> Hapus',array('class'=>'cekboxaction ui btn btn-warning','confirm'=>'Apa anda yakin?'))?>
</p>
